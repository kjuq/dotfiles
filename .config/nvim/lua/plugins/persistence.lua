...
spec.keys = {
	map('s', 'n', function() require('persistence').load() end, 'Restore session for the current dir'),
	map('l', 'n', function() require('persistence').load({ last = true }) end, 'Restore the last session'),
	map('p', 'n', function() require('persistence').select() end, 'Pick a session to load'),
...
spec.config = function(_, opts)
	...
	vim.api.nvim_create_autocmd({ 'User' }, {
		...
		callback = function()
			...
			if _G._user_init_cwd then vim.fn.chdir(_G._user_init_cwd) end
		end,
	})
	vim.api.nvim_create_autocmd({ 'User' }, {
		pattern = 'PersistenceSavePost',
		group = vim.api.nvim_create_augroup('user_persistence_save_post', {}),
		callback = function() vim.fn.chdir(dir_bak) end,
	})
	...
	if load_session then
		local delay_ms = 0
		vim.defer_fn(function() require('persistence').load() end, delay_ms)
	end
end
